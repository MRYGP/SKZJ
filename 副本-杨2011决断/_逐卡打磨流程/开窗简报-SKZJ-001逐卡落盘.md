...
## 当前态（commit 14f5501 第一可跑版已落；2026-06-28 已同步停顿〇 v1.3 + 第三幕 v1.5 八卡）
- `副本-杨2011决断\h5\index.html` ＝ v1.1 第一可跑版骨架：卡1-22 + card-15b + card-16b；停顿〇已改为两身体声音触发键（点任一键→交替二次→卡住→卡11；无反馈、不判对错）；第三幕卡16/16b/17a/17b/18/18b/19/20已同步为 v1.5 八卡决策链；音频引擎 `bedAudio/bgmAudio/activeAudio(旁白)/silent` + `unlockAudio()` 微信解锁 + pause/resume 同步全层；每卡 `bg` 走 CSS 渐变 class；卡22 止（无第二阶段）。
- 卡21：`type:photo` + `audioFull: assets/audio/card21.m4a`（2026-06-28 零我版技术精剪，109.500s）+ `img: assets/img/card21-photo.png`，`fixed:109500`（降级时长）。
...
5. **卡21 二剪后**：如替换成品音频，`fixed` 按实际时长重改。
