...
- `副本-杨2011决断\h5\index.html` ＝ v1.1 第一可跑版骨架：卡1-22；停顿〇已改为两身体声音触发键（点任一键→交替二次→卡住→卡11；无反馈、不判对错）；第三幕卡16-20已同步为 v1.4 七卡决策链；音频引擎 `bedAudio/bgmAudio/activeAudio(旁白)/silent` + `unlockAudio()` 微信解锁 + pause/resume 同步全层；每卡 `bg` 走 CSS 渐变 class；卡22 止（无第二阶段）。
